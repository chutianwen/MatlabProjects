...
theta1_current=-25*pi/180;
theta2_current=40*pi/180;
theta3_current=20*pi/180;
theta4_current=25*pi/180;
theta5_current=10*pi/180;
theta6_current=35*pi/180;
...
p3_yto2=p3_y-p2_y;
p3_zto2=p3_z-p2_z;
apha=pi/2;   % pi/2     1.184677180994306
% syms apha;
r=[1       0        0;
   0    cos(apha)  sin(apha);
   0    -sin(apha)  cos(apha)];
...
delta_theta=pinv(J)*dif_tar;
theta1_new=theta_current+delta_theta;
theta_current=theta1_new;

i=i+1;
end
result_theta=theta_current;
num_2pi=round(result_theta/(2*pi));
result_theta=result_theta-num_2pi*2*pi;

% result_theta=theta_current*180/pi;
% num_2pi=round(result_theta/360);
% result_theta=result_theta-num_2pi*360;